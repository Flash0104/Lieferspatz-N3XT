...
function RestaurantDashboard() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh orders every 5 seconds for fast updates
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--background)' }}>
        <div className="text-center">
          ...
          <div className="text-xl" style={{ color: 'var(--foreground)' }}>Loading restaurant dashboard...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--background)' }}>
        <div className="text-center">
          ...
          <div className="mb-4" style={{ color: 'var(--foreground)', opacity: '0.7' }}>Your restaurant profile may not be set up yet.</div>
          <button 
  ...
  return (
    <div className="min-h-screen pt-20" style={{ backgroundColor: 'var(--background)' }}>
      <div className="max-w-6xl mx-auto px-6 py-8">
        <h1 className="text-3xl font-bold mb-8" style={{ color: 'var(--foreground)' }}>Restaurant Dashboard</h1>
        
        {/* Restaurant Info Card */}
        <div className="rounded-lg shadow-lg p-6 mb-8" style={{ backgroundColor: 'var(--card)' }}>
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-2xl font-bold mb-2" style={{ color: 'var(--foreground)' }}>{restaurant.name}</h2>
              <p style={{ color: 'var(--foreground)', opacity: '0.7' }}>{restaurant.address}, {restaurant.city}</p>
              <div className="flex items-center mt-2">
                <span className="text-yellow-500">★</span>
                <span className="ml-1" style={{ color: 'var(--foreground)' }}>{restaurant.rating.toFixed(1)} rating</span>
              </div>
              <div className="mt-4">
            ...
            <div className="text-right">
              <div className="px-4 py-2 rounded-lg mb-4" style={{ backgroundColor: 'var(--background)' }}>
                <div className="text-sm" style={{ color: 'var(--foreground)', opacity: '0.7' }}>Restaurant Balance</div>
        ...
        <div className="rounded-lg shadow-lg p-6 mb-8" style={{ backgroundColor: 'var(--card)' }}>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold" style={{ color: 'var(--foreground)' }}>Menu Items ({menuItems.length})</h3>
            <button 
              onClick={() => router.push('/profile/restaurant')}
          ...
          {menuItems.length === 0 ? (
            <div className="text-center py-8">
              <p className="mb-4" style={{ color: 'var(--foreground)', opacity: '0.6' }}>No menu items yet</p>
              <button 
                onClick={() => router.push('/profile/restaurant')}
                className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {menuItems.slice(0, 6).map((item) => (
                <div key={item.id} className="border rounded-lg p-4">
                  ...
                  />
                  <h4 className="font-semibold" style={{ color: 'var(--foreground)' }}>{item.name}</h4>
                  <p className="text-sm" style={{ color: 'var(--foreground)', opacity: '0.7' }}>{item.description}</p>
                  <p className="text-teal-600 font-bold">€{item.price.toFixed(2)}</p>
                  <p className="text-sm" style={{ color: 'var(--foreground)', opacity: '0.6' }}>Category: {item.category}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="rounded-lg shadow p-6" style={{ backgroundColor: 'var(--card)' }}>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium" style={{ color: 'var(--foreground)', opacity: '0.7' }}>Pending Orders</p>
                <p className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>{orders.filter(o => o.status === 'PENDING').length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg shadow p-6" style={{ backgroundColor: 'var(--card)' }}>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium" style={{ color: 'var(--foreground)', opacity: '0.7' }}>In Progress</p>
                <p className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>{orders.filter(o => ['ACCEPTED', 'PREPARING'].includes(o.status)).length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg shadow p-6" style={{ backgroundColor: 'var(--card)' }}>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium" style={{ color: 'var(--foreground)', opacity: '0.7' }}>Completed Today</p>
                <p className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>{orders.filter(o => o.status === 'DELIVERED').length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg shadow p-6" style={{ backgroundColor: 'var(--card)' }}>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium" style={{ color: 'var(--foreground)', opacity: '0.7' }}>Today's Revenue</p>
                <p className="text-2xl font-bold" style={{ color: 'var(--foreground)' }}>€{orders.filter(o => o.status === 'DELIVERED').reduce((sum, o) => sum + o.totalPrice, 0).toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Active Orders */}
          <div className="rounded-lg shadow-lg p-6" style={{ backgroundColor: 'var(--card)' }}>
            <h3 className="text-xl font-bold mb-4" style={{ color: 'var(--foreground)' }}>Active Orders ({pendingOrders.length})</h3>
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {pendingOrders.length === 0 ? (
                <p className="text-center py-8" style={{ color: 'var(--foreground)', opacity: '0.6' }}>No active orders</p>
              ) : (
          ...
          {/* Completed Orders */}
          <div className="rounded-lg shadow-lg p-6" style={{ backgroundColor: 'var(--card)' }}>
            <h3 className="text-xl font-bold mb-4" style={{ color: 'var(--foreground)' }}>Recent Completed Orders</h3>
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {completedOrders.length === 0 ? (
                <p className="text-center py-8" style={{ color: 'var(--foreground)', opacity: '0.6' }}>No completed orders yet</p>
              ) : (
                  ...
                  <div key={order.id} className={`border-l-4 pl-4 py-2 ${getStatusColor(order.status)}`}>
                    ...
                    <div className="mt-2">
                      ...
                      {order.orderItems.length > 2 && (
                        <p className="text-xs text-gray-500 mt-1">+{order.orderItems.length - 2} more items</p>
                      )}
                    </div>
                  </div>
                ))
